...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItems = @[customSaveButton,rightItemh];
...
- (void)initVideoCutView:(VideoType)type
{
    ...
    if (type == VideoType_Video) {
        ...
        _videoCutView = [[VideoCutView alloc] initWithFrame:frame videoPath:_videoPath videoAssert:_videoAsset];
        flag = YES;
         [self.view addSubview:_videoCutView];
    }else{
        if (_videoCutView) {
            ...
            _videoCutView = [[VideoCutView alloc] initWithFrame:frame pictureList:_imageList duration:_duration];
            flag = YES;
            [self.view addSubview:_videoCutView];
        }
    }
