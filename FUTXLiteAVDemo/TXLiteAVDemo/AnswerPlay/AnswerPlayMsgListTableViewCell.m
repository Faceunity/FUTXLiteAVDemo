//
//  AnswerPlayMsgListTableViewCell.m
//  TXLiteAVDemo
//
...
@implementation AnswerPlayMsgModel

@end


@implementation AnswerPlayMsgListTableViewCell
{
    UIView             *_msgView;
    UILabel            *_msgLabel;
    
    AnswerPlayMsgModeType     _msgModeType;
}
...
- (void)layoutSubviews {
    ...
    if (_msgModeType == AnswerPlayMsgModeTypeOneself) {
...
+ (NSAttributedString *)getAttributedStringFromModel:(AnswerPlayMsgModel *)msgModel {
    ...
    // 系统消息
    if (msgModel.type == AnswerPlayMsgModeTypeSystem) {
    ...
    UIColor *color = nil;
    if (msgModel.type == AnswerPlayMsgModeTypeOneself) {
