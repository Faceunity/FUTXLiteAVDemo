...
@implementation RTCMsgModel

@end


@implementation RTCMsgListTableViewCell
{
    UIView             *_msgView;
    UILabel            *_msgLabel;
    
    RTCMsgModeType     _msgModeType;
}
...
- (void)layoutSubviews {
    ...
    if (_msgModeType == RTCMsgModeTypeOneself) {
...
+ (NSAttributedString *)getAttributedStringFromModel:(RTCMsgModel *)msgModel {
    ...
    // 系统消息
    if (msgModel.type == RTCMsgModeTypeSystem) {
    ...
    UIColor *color = nil;
    if (msgModel.type == RTCMsgModeTypeOneself) {
